...
const STEPS = [
  {
    ...
    visual: (
      <div className="bg-ink-deep rounded-xl p-6 space-y-4">
        <div className="flex gap-4">
          <div className="flex-1 bg-white/5 rounded-lg p-4 border border-white/8">
        ...
        <div className="bg-white/5 rounded-lg p-4 border border-white/8">
          ...
          {[
            ...
            <div key={item.name} className="flex items-center gap-3 mb-2.5 last:mb-0">
              ...
              <span className="text-white/40 text-xs w-14 text-right">{item.score}% <span className="text-white/25">Score</span></span>
            </div>
          ))}
        </div>
      </div>
        ...
        <div className="flex items-center gap-4 mb-2">
          ...
          <div className="flex gap-2">
            <div className="bg-white/5 rounded px-2 py-1 border border-white/8">
              <span className="text-blue-bright text-lg font-bold">3,6K</span>
              <span className="text-white/30 text-xs ml-1">Data Points</span>
            </div>
            <div className="bg-white/5 rounded px-2 py-1 border border-white/8">
              <span className="text-blue-bright text-lg font-bold">2m</span>
              <span className="text-white/30 text-xs ml-1">Processing</span>
            </div>
          </div>
        </div>
        <div className="bg-white/5 rounded-lg p-4 border border-white/8 font-mono text-sm">
          ...
          <p className="text-white/40 pl-8">model: &apos;analysis&apos;,</p>
      ...
      <div className="bg-ink-deep rounded-xl p-6 space-y-4">
        <div className="flex items-center gap-4 mb-2">
          <div className="flex gap-2">
            <div className="bg-white/5 rounded px-2 py-1 border border-white/8">
              <span className="text-blue-bright text-lg font-bold">3,6K</span>
              <span className="text-white/30 text-xs ml-1">Users</span>
            </div>
            <div className="bg-white/5 rounded px-2 py-1 border border-white/8">
              <span className="text-blue-bright text-lg font-bold">2m</span>
              <span className="text-white/30 text-xs ml-1">Processed</span>
            </div>
          </div>
        </div>
        <div className="bg-white/5 rounded-lg p-4 border border-white/8">
    ...
    <section id="process" className="bg-white section">
      <div className="container">
        ...
        <ScrollReveal animation="fade-in-scale">
          <div className="max-w-5xl mx-auto bg-gray-900 rounded-3xl overflow-hidden">
            {/* Step tabs */}
            <div className="grid grid-cols-3">
              {STEPS.map((step, i) => (
                <button
                  key={step.label}
                  onClick={() => setActive(i)}
                  className={`py-4 text-sm font-semibold tracking-wide uppercase transition-all duration-300 cursor-pointer ${
                    active === i
